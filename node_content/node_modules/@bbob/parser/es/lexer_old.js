    ...
    const next = ()=>{
        ...
        if (currChar === N) {
            ...
            if (isCharReserved(nextChar) || hasInvalidChars || bufferGrabber.isLast()) {
                ...
                const str1 = bufferGrabber.grabWhile((val)=>val !== closeTag);
                bufferGrabber.skip(); // skip closeTag
                // [myTag   ]
                const isNoAttrsInTag = str1.indexOf(EQ) === -1;
                // [/myTag]
                const isClosingTag = str1[0] === SLASH;
                if (isNoAttrsInTag || isClosingTag) {
                    emitToken(createToken(TYPE_TAG, str1, row, col));
                } else {
                    const parsed = parseAttrs(str1);
            ...
            if (escapeTags && isEscapeChar(currChar) && !isEscapableChar(nextChar)) {
                ...
                const str2 = bufferGrabber.grabWhile((char)=>{
                    if (escapeTags) {
                        return isCharToken(char) && !isEscapeChar(char);
                    }
                    return isCharToken(char);
                });
                emitToken(createToken(TYPE_WORD, str2, row, col));
            }
        }
    };
    const tokenize = ()=>{
